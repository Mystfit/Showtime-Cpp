...
namespace showtime {
	...
	void ZstDynamicValue::take(int* newData, size_t count)
	{
		assign(newData, count);
	}

	void ZstDynamicValue::take(float* newData, size_t count)
	{
		assign(newData, count);
	}

	void ZstDynamicValue::take(char** newData, size_t count)
	{
		assign_strings(const_cast<const char**>(newData), count);
	}

	void ZstDynamicValue::take(uint8_t* newData, size_t count)
	{
		assign(newData, count);
	}

	void* ZstDynamicValue::release()
	{
		clear();

		// Dynamic values own their data - nothing to release
		return nullptr;
	}
