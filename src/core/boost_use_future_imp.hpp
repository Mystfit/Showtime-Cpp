...
inline void promise_invoke_and_set(boost::promise<T>& p,
...
inline void promise_invoke_and_set(boost::promise<void>& p,
    F& f, BOOST_ASIO_MOVE_ARG(Args)... args)
{
#if !defined(BOOST_ASIO_NO_EXCEPTIONS)
  try
...
inline void promise_invoke_and_set(boost::promise<T>& p, F& f)
{
#if !defined(BOOST_ASIO_NO_EXCEPTIONS)
  try
#endif // !defined(BOOST_ASIO_NO_EXCEPTIONS)
...
template <typename F, typename Args>
inline void promise_invoke_and_set(boost::promise<void>& p, F& f)
{
#if !defined(BOOST_ASIO_NO_EXCEPTIONS)
  try
#endif // !defined(BOOST_ASIO_NO_EXCEPTIONS)
...
{
public:
  promise_invoker(const shared_ptr<boost::promise<T> >& p,
...
{
public:
  explicit promise_executor(const shared_ptr<boost::promise<T> >& p)
    : p_(p)
  {
  }
...
public:
  ...
  promise_handler(use_boost_future_t<Allocator> u)
