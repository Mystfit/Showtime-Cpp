...
void ZstClientTransport::process_events()
{
	ZstTransportLayerBase::process_events();
	ZstMessageSupervisor::cleanup_response_messages();
}

void ZstClientTransport::begin_send_message(ZstMessage * msg, const ZstTransportSendType & sendtype, const MessageReceivedAction & action)
{
	if (!msg) return;
...
void ZstClientTransport::sock_recv(zsock_t* socket, bool pop_first)
{
	if (!is_active())
		return;
...
ZstMessageReceipt ZstClientTransport::send_sync_message(ZstStageMessage * msg)
{
...
void ZstClientTransport::send_async_message(ZstStageMessage * msg, const MessageReceivedAction & completed_action)
{
	auto future = register_response(msg->id());
