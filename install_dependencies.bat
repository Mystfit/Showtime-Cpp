...
IF NOT DEFINED BUILD_FOLDER set BUILD_FOLDER=C:\projects\showtime-cpp

REM Set up dependency directories
pushd %BUILD_FOLDER%

IF NOT DEFINED DEPENDENCY_DIR set DEPENDENCY_DIR=%BUILD_FOLDER%\dependencies
IF NOT EXIST %DEPENDENCY_DIR% (
    mkdir "%DEPENDENCY_DIR%"
    mkdir "%DEPENDENCY_DIR%\install"
)
IF NOT DEFINED HUNTER_ROOT set HUNTER_ROOT=%DEPENDENCY_DIR%\hunter_root
IF NOT EXIST %HUNTER_ROOT% mkdir "%HUNTER_ROOT%"
